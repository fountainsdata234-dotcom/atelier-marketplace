  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      <section className={`overflow-hidden rounded-3xl border ${surface}`}>
        <div className="relative overflow-hidden bg-gradient-to-br from-amber-500/20 via-neutral-950 to-orange-900/20 p-6 sm:p-10">
          <div className="relative flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
            <div className="flex items-center gap-4">
              {seller.avatarUrl ? <img src={seller.avatarUrl} alt="" className="h-20 w-20 rounded-2xl object-cover ring-2 ring-amber-400/60" /> : <div className="flex h-20 w-20 items-center justify-center rounded-2xl bg-amber-400/20 text-2xl font-black text-amber-300">{seller.name.slice(0, 2).toUpperCase()}</div>}
              <div><p className="text-[10px] font-bold uppercase tracking-[0.22em] text-amber-300">Verified atelier profile</p><h1 className="mt-1 text-3xl font-serif font-bold text-white sm:text-4xl">{seller.shopName || seller.name}</h1><p className="text-xs text-amber-100/70">{seller.handle} · {seller.role.replace('_', ' ')}</p></div>
            </div>
        ...
        <div className="p-5 sm:p-8"><div className="mb-5 flex items-end justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-amber-500">Collection</p><h2 className="mt-1 text-2xl font-serif font-bold">Published work</h2></div><span className="text-xs text-neutral-400">{sellerPosts.length} live post{sellerPosts.length === 1 ? '' : 's'}</span></div>{sellerPosts.length === 0 ? <p className="rounded-2xl border border-dashed border-neutral-700 p-8 text-center text-xs text-neutral-400">This seller has not published a collection yet.</p> : <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{sellerPosts.map(post => <article key={post.id} className={`overflow-hidden rounded-2xl border ${isDarkMode ? 'border-neutral-800 bg-neutral-950/60' : 'border-neutral-200 bg-neutral-50'}`}><img src={post.imageUrl} alt={post.title} className="aspect-[4/3] w-full object-cover" /><div className="p-4"><h3 className="font-serif text-lg font-bold">{post.title}</h3><p className="mt-1 line-clamp-2 text-xs text-neutral-400">{post.description}</p><div className="mt-3 flex items-center justify-between text-[10px] text-neutral-400"><span>{post.likes?.length || 0} likes</span><span>{post.saves?.length || 0} saves</span><span>{post.rating ? `${post.rating.toFixed(1)} rating` : 'New'}</span></div></div></article>)}</div>}</div>
      </section>
    </main>
  );
};
